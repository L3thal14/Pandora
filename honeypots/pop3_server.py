    ...
    def pop3_server_main(self):
        ...
        class CustomPOP3Protocol(POP3):
            ...
            def connectionMade(self):
                _q_s.logs.info({'server': 'pop3_server', 'action': 'connection', 'ip': self.transport.getPeer(
                ).host, 'port': self.transport.getPeer().port})
                self._user = None
                if isinstance(_q_s.mocking, bool):
                    if _q_s.mocking == True:
                        self.successResponse('{}'.format(
                            choice(_q_s.random_servers)))
            ...
            def do_PASS(self, password):
                if self._user:
                    if self._user.decode() == _q_s.username and password.decode() == _q_s.password:
                        _q_s.logs.info({'server': 'pop3_server', 'action': 'login', 'status': 'success', 'ip': self.transport.getPeer(
                        ).host, 'port': self.transport.getPeer().port, 'username': _q_s.username, 'password': _q_s.password})
                    else:
                        _q_s.logs.info({'server': 'pop3_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'pop3_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'pop3_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'pop3_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qpop3server = QPOP3Server(ip=parsed.ip, port=parsed.port, username=parsed.username,
