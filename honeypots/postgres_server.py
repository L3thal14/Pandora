...
class QPostgresServer():
    ...
    def postgres_server_main(self):
        ...
        class CustomPostgresProtocol(Protocol):
            ...
            def connectionMade(self):
                self._state = 1
                self._variables = {}
                _q_s.logs.info({'server': 'postgres_server', 'action': 'connection',
                ...
                if self._state == 1:
                    ...
                    if data[0] == 112 and "user" in self._variables:
                        ...
                        if self._variables["user"] == _q_s.username and self._variables["password"] == _q_s.password:
                            _q_s.logs.info({'server': 'postgres_server', 'action': 'login', 'status': 'success', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': _q_s.username, 'password': _q_s.password})
                        else:
                            _q_s.logs.info({'server': 'postgres_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': self._variables["user"], 'password': self._variables["password"]})
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'postgres_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'postgres_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'postgres_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def close_port(self):
        ret = close_port_wrapper(
            'postgres_server', self.ip, self.port, self.logs)
        return ret
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qpostgresserver = QPostgresServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
