...
class QSOCKS5Server():
    ...
    def socks5_server_main(self):
        ...
        class CustomStreamRequestHandler(StreamRequestHandler):
            def handle(self):
                _q_s.logs.info({'server': 'socks5_server', 'action': 'connection',
                    ...
                    if 2 in unpack("!" + "B" * m, self.connection.recv(m)):
                        ...
                        if 1 in unpack("B", self.connection.recv(1)):
                            ...
                            if username.decode() == _q_s.username and password.decode() == _q_s.password:
                                _q_s.logs.info({'server': 'socks5_server', 'action': 'login', 'status': 'success',
                                                'ip': self.client_address[0], 'port': self.client_address[1], 'username': _q_s.username, 'password': _q_s.password})
                            else:
                                _q_s.logs.info({'server': 'socks5_server', 'action': 'login', 'status': 'failed', 'ip': self.client_address[
                                               0], 'port': self.client_address[1], 'username': username.decode(), 'password': password.decode()})
        ...
        server = ThreadingTCPServer(
            (self.ip, self.port), CustomStreamRequestHandler)
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'socks5_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'socks5_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'socks5_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            get('https://yahoo.com', proxies=dict(http='socks5://{}:{}@{}:{}'.format(_username, _password,
                                                                                     _ip, _port), https='socks5://{}:{}@{}:{}'.format(_username, _password, _ip, _port)))
        except BaseException:
            pass

    def close_port(self):
        ret = close_port_wrapper(
            'socks5_server', self.ip, self.port, self.logs)
        return ret
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        QSOCKS5Server = QSOCKS5Server(ip=parsed.ip, port=parsed.port, username=parsed.username,
