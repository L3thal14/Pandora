...
class QHTTPSServer():
    def __init__(self, ip=None, port=None, username=None, password=None, mocking=False, config=''):
        ...
        self.random_servers = ['Apache', 'nginx', 'Microsoft-IIS/7.5', 'Microsoft-HTTPAPI/2.0', 'Apache/2.2.15', 'SmartXFilter', 'Microsoft-IIS/8.5', 'Apache/2.4.6', 'Apache-Coyote/1.1',
                               'Microsoft-IIS/7.0', 'Apache/2.4.18', 'AkamaiGHost', 'Apache/2.2.25', 'Microsoft-IIS/10.0', 'Apache/2.2.3', 'nginx/1.12.1', 'Apache/2.4.29', 'cloudflare', 'Apache/2.2.22']
        self.process = None
...
'''
            ...
            def render(self, request):
                ...
                try:
                    ...
                    for item, value in dict(request.requestHeaders.getAllRawHeaders()).items():
                        headers.update(
                            {check_bytes(item): ','.join(map(check_bytes, value))})
                except BaseException:
                    pass

                _q_s.logs.info({'server': 'https_server', 'action': 'connection',
                                'ip': request.getClientIP(), 'request': headers})
                ...
                if request.method == b"GET":
                    _q_s.logs.info(
                        {'server': 'https_server', 'action': 'get', 'ip': request.getClientIP()})
                    ...
                    _q_s.logs.info(
                        {'server': 'https_server', 'action': 'post', 'ip': request.getClientIP()})
                    if request.uri == b"/login.html" or b'/':
                        if _q_s.username != '' and _q_s.password != '':
                            form = FieldStorage(fp=request.content, headers=self.headers, environ={
                                                'REQUEST_METHOD': 'POST', 'CONTENT_TYPE': self.headers[b'content-type'], })
                            if 'username' in form and 'password' in form:
                                if form['username'].value == _q_s.username and form['password'].value == _q_s.password:
                                    _q_s.logs.info({'server': 'https_server', 'action': 'login', 'status': 'success', 'ip': request.getClientIP(
                                    ), 'username': _q_s.username, 'password': _q_s.password})
                                else:
                                    _q_s.logs.info({'server': 'https_server', 'action': 'login', 'status': 'failed', 'ip': request.getClientIP(
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'https_server', 'action': 'process', 'status': 'success', 'route': '/login.html',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'https_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'https_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            post('https://{}:{}'.format(_ip, _port), data={'username': (None,
                                                                        _username), 'password': (None, _password)}, verify=False)
        except BaseException:
            pass
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qhttpsserver = QHTTPSServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
