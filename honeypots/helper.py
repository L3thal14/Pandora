...
def get_running_servers():
    temp_list = []
    try:
        honeypots = ['QDNSServer', 'QFTPServer', 'QHTTPProxyServer', 'QHTTPServer', 'QHTTPSServer', 'QIMAPServer', 'QMysqlServer', 'QPOP3Server',
...
def setup_logger(temp_name, config, drop=False):
    ...
    if 'terminal' in logs or 'db' in logs or 'all' in logs or logs == '':
        if 'db' in logs or 'all' in logs:
            ret_logs_obj.addHandler(CustomHandler(
                temp_name, logs, config_data, drop))
    ...
    if 'file' in logs or 'all' in logs:
        ...
        file_handler = RotatingFileHandler(
            path.join(logs_location, temp_name), maxBytes=10000, backupCount=10)
...
def close_port_wrapper(server_name, ip, port, logs):
    ...
    if sock.connect_ex((ip, port)) != 0:
        ...
        logs.error({'server': server_name, 'error': 'port_open',
                    'type': 'Port {} still open..'.format(ip)})
        return False
...
class CustomHandler(Handler):
    def __init__(self, uuid='', logs='', config=None, drop=False):
        ...
        self.uuid = uuid
        if config and config != '':
            self.db = postgres_class(host=config['host'], port=config['port'], username=config['username'],
                                     password=config['password'], db=config['db'], uuid=self.uuid, drop=drop)
    ...
    def emit(self, record):
        try:
            if self.logs == 'all' or self.logs == 'db':
                if self.db:
                    self.db.insert_into_data_safe(record.msg[0], dumps(
                        serialize_object(record.msg[1]), cls=ComplexEncoder))
        except Exception as e:
            stdout.write(dumps({"error": repr(e), "logger": repr(
                record)}, sort_keys=True, cls=ComplexEncoder) + "\n")
        try:
            if self.logs == 'all' or self.logs == '':
                ...
                if record.msg[0] == "servers":
                    if "server" in record.msg[1]:
                        ...
                        server = temp['server'].replace(
                            'server', '').replace('_', '')
                        del temp['server']
                        del temp['action']
                        stdout.write("[{}] [{}] [{}] -> {}\n".format(time_now, server,
                                                                     action, dumps(temp, sort_keys=True, cls=ComplexEncoder)))
            else:
                stdout.write(dumps(record.msg, sort_keys=True,
                                   cls=ComplexEncoder) + "\n")
        except Exception as e:
            stdout.write(dumps({"error": repr(e), "logger": repr(
                record)}, sort_keys=True, cls=ComplexEncoder) + "\n")
        ...
        try:
            if record.msg[0] == "servers":
                if "server" in record.msg[1]:
                    ...
                    server = temp['server']
                    del temp['server']
                    stdout.write(highlight(dumps({server: temp}, sort_keys=True, indent=4,
                                                 cls=ComplexEncoder), lexers.JsonLexer(), formatters.TerminalFormatter()))
            else:
                stdout.write(highlight(dumps(record.msg, sort_keys=True, indent=4,
                                             cls=ComplexEncoder), lexers.JsonLexer(), formatters.TerminalFormatter()))
        except Exception as e:
            stdout.write(highlight(dumps({"error": repr(e), "logger": repr(
                record)}, sort_keys=True, indent=4, cls=ComplexEncoder), lexers.JsonLexer(), formatters.TerminalFormatter()))
        stdout.flush()
...
class postgres_class():
    def __init__(self, host=None, port=None, username=None, password=None, db=None, drop=False, uuid=None):
        ...
        self.wait_until_up()
        if drop:
            self.con = connect(host=self.host, port=self.port,
            ...
            self.con.close()
        self.con = connect(host=self.host, port=self.port,
                           user=self.username, password=self.password, database=self.db)
        self.con.set_isolation_level(0)
        self.cur = self.con.cursor()
        self.create_tables()
    ...
    def wait_until_up(self):
        ...
        while test:
            try:
                ...
                stdout.flush()
                conn = connect(host=self.host, port=self.port, user=self.username,
                               password=self.password, connect_timeout=1)
                conn.close()
                test = False
            except Exception as e:
                pass
    ...
    def check_db_if_exists(self):
        self.cur.execute(
            "SELECT exists(SELECT 1 from pg_catalog.pg_database where datname = %s)", (self.db,))
    ...
    def drop_tables(self,):
        for x in self.mapped_tables:
            self.cur.execute(sql.SQL("drop TABLE IF EXISTS {}").format(
                sql.Identifier(x + "_table")))

    def create_tables(self):
        for x in self.mapped_tables:
            self.cur.execute(sql.SQL("CREATE TABLE IF NOT EXISTS {} (id SERIAL NOT NULL,date timestamp with time zone DEFAULT now(),data json)").format(
                sql.Identifier(x + "_table")))
...
def server_arguments():
    ...
    _server_parsergroupdeq = _server_parser.add_argument_group(
        'Initialize Server')
    ...
    _server_parsergroupdeq.add_argument(
        '--resolver_addresses', type=str, help="Change resolver address", required=False, metavar='')
    _server_parsergroupdeq.add_argument(
        '--domain', type=str, help="A domain to test", required=False, metavar='')
    _server_parsergroupdeq.add_argument(
        '--mocking', type=str, help="Random banner", required=False)
    _server_parsergroupdes = _server_parser.add_argument_group(
        'Sinffer options')
    ...
    _server_parsergroupdef = _server_parser.add_argument_group(
        'Initialize Loging')
    _server_parsergroupdef.add_argument(
        '--config', type=str, help="config file for logs and database", required=False, default="")
    _server_parsergroupdea = _server_parser.add_argument_group(
        'Auto Configuration')
