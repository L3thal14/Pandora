...
class QMysqlServer():
    ...
    def greeting(self):
        base = ['\x0a', '5.7.00' + '\0', '\x36\x00\x00\x00', '12345678' + '\0', '\xff\xf7', '\x21',
                '\x02\x00', '\x0f\x81', '\x15', '\0' * 10, '123456789012' + '\0', 'mysql_native_password' + '\0']
        payload_len = list(pack('<I', len(''.join(base))))
        # payload_len[3] = '\x00'
        string_ = chr(payload_len[0]) + chr(payload_len[1]) + \
            chr(payload_len[2]) + '\x00' + ''.join(base)
        return string_
    ...
    def too_many(self):
        ...
        # payload_len[3] = '\x02'
        string_ = chr(payload_len[0]) + chr(payload_len[1]) + \
            chr(payload_len[2]) + '\x02' + ''.join(base)
        return string_
    ...
    def parse_data(self, data):
        ...
        try:
            ...
            password = data[36 + username_len +
                            2:36 + username_len + 2 + password_len]
    ...
    def decode(self, hash):
        ...
        try:
            for word in self.words:
                ...
                encrypted = ''.join(chr((a) ^ (b)) for a, b in zip(
                    hash1, sha1(b'12345678123456789012' + hash2).digest()))
                if encrypted == hash:
                    return temp
        except Exception as e:
            pass
    ...
    def mysql_server_main(self):
        ...
        class CustomMysqlProtocol(Protocol):
            ...
            def connectionMade(self):
                ...
                _q_s.logs.info({'server': 'mysql_server', 'action': 'connection', 'ip': self.transport.getPeer(
                    ...
                    if good:
                        if password:
                            ...
                            if _x == _q_s.password and _x is not None:
                                _q_s.logs.info({'server': 'mysql_server', 'action': 'login', 'status': 'success', 'ip': self.transport.getPeer(
                                ).host, 'port': self.transport.getPeer().port, 'username': _q_s.username, 'password': _q_s.password})
                            else:
                                _q_s.logs.info({'server': 'mysql_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                                ).host, 'port': self.transport.getPeer().port, 'username': username, 'password': ':'.join(hex((c))[2:] for c in password)})
                        else:
                            _q_s.logs.info({'server': 'mysql_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': 'UnKnown', 'password': ':'.join(hex((c))[2:] for c in data)})
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'mysql_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'mysql_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'mysql_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            cnx = mysqlconnect(user=_username, password=_password, host=_ip,
                               port=_port, database='test', connect_timeout=1000)
            print("31323221s")
        except Exception as e:
            pass
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qmysqlserver = QMysqlServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
