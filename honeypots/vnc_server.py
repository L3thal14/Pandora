...
class QVNCServer():
    ...
    def decode(self, c, r):
        try:
            for word in self.words:
                ...
                for i in range(0, 8):
                    rev_word.append(
                        chr(int('{:08b}'.format(ord(word[i]))[::-1], 2)))
                output = DES.new(''.join(rev_word).encode(
                    'utf-8'), DES.MODE_ECB).encrypt(c)
                if output == r:
                    return temp
    ...
    def vnc_server_main(self):
        ...
        class CustomVNCProtocol(Protocol):
            ...
            def connectionMade(self):
                ...
                self._state = 1
                _q_s.logs.info({'server': 'vnc_server', 'action': 'connection', 'ip': self.transport.getPeer(
            ...
            def dataReceived(self, data):
                if self._state == 1:
                    ...
                    if _x:
                        if _x == _q_s.password:
                            _q_s.logs.info({'server': 'vnc_server', 'action': 'login', 'status': 'success', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': 'UnKnown', 'password': _q_s.password})
                        else:
                            _q_s.logs.info({'server': 'vnc_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': 'UnKnown', 'password': _x})
                    else:
                        if len(data) == 16:
                            # we need to check the lenth check length first
                            _q_s.logs.info({'server': 'vnc_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                            ).host, 'port': self.transport.getPeer().port, 'username': 'UnKnown', 'password': data.hex()})
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'vnc_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'vnc_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'vnc_server', 'action': 'setup', 'status': 'error', 'ip': self.ip,
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            client = vncapi.connect("{}::{}".format(
                self.ip, self.port), password=password)
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qvncserver = QVNCServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
