...
class QSSHServer():
    def __init__(self, ip=None, port=None, username=None, password=None, mocking=False, config=''):
        ...
        self.random_servers = ['OpenSSH 7.5', 'OpenSSH 7.3',
    ...
    def ssh_server_main(self):
        ...
        class SSHHandle(ServerInterface):
            ...
            def check_auth_password(self, username, password):
                if username == _q_s.username and password == _q_s.password:
                    _q_s.logs.info({'server': 'ssh_server', 'action': 'login', 'status': 'success',
                                    'ip': self.ip, 'port': self.port, 'username': username, 'password': password})
                else:
                    _q_s.logs.info({'server': 'ssh_server', 'action': 'login', 'status': 'failed',
                                    'ip': self.ip, 'port': self.port, 'username': username, 'password': password})
        ...
        def ConnectionHandle(client, priv):
            try:
                ...
                _q_s.logs.info(
                    {'server': 'ssh_server', 'action': 'connection', 'ip': ip, 'port': port})
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'ssh_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'ssh_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'ssh_server', 'action': 'setup', 'status': 'error', 'ip': self.ip,
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            ssh.connect(_ip, port=_port, username=_username,
                        password=_password)
        except BaseException:
            pass
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qsshserver = QSSHServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
