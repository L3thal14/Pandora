...
'''
            ...
            def render(self, request):
                ...
                _q_s.logs.info({'server': 'http_server', 'action': 'connection',
                ...
                if request.method == b"GET":
                    _q_s.logs.info(
                        {'server': 'http_server', 'action': 'get', 'ip': request.getClientIP()})
                    ...
                    _q_s.logs.info(
                        {'server': 'http_server', 'action': 'post', 'ip': request.getClientIP()})
                        ...
                        if _q_s.username != '' and _q_s.password != '':
                            ...
                            if 'username' in form and 'password' in form:
                                if form['username'].value == _q_s.username and form['password'].value == _q_s.password:
                                    _q_s.logs.info({'server': 'http_server', 'action': 'login', 'status': 'success', 'ip': request.getClientIP(
                                    ), 'username': _q_s.username, 'password': _q_s.password})
                                else:
                                    _q_s.logs.info({'server': 'http_server', 'action': 'login', 'status': 'failed', 'ip': request.getClientIP(
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'http_server', 'action': 'process', 'status': 'success', 'route': '/login.html',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'http_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'http_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def test_server(self, ip=None, port=None, username=None, password=None):
        try:
            ...
            post('http://{}:{}/login.html'.format(_ip, _port),
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qhttpserver = QHTTPServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
