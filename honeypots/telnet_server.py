...
class QTelnetServer():
    def __init__(self, ip=None, port=None, username=None, password=None, mocking=False, config=''):
        ...
        self.random_servers = [
            'Ubuntu 18.04 LTS', 'Ubuntu 16.04.3 LTS', 'Welcome to Microsoft Telnet Server.']
        self.process = None
    ...
    def telent_server_main(self):
        ...
        class CustomTelnetProtocol(TelnetProtocol):
            ...
            def connectionMade(self):
                ...
                _q_s.logs.info({'server': 'telnet_server', 'action': 'connection', 'ip': self.transport.getPeer(
            ...
            def dataReceived(self, data):
                ...
                if self._state == b'Username':
                    ...
                    if self._user.decode() == _q_s.username and self._pass.decode() == _q_s.password:
                        _q_s.logs.info({'server': 'telnet_server', 'action': 'login', 'status': 'success', 'ip': self.transport.getPeer(
                        ).host, 'port': self.transport.getPeer().port, 'username': _q_s.username.decode('utf-8'), 'password': _q_s.password.decode('utf-8')})
                    else:
                        _q_s.logs.info({'server': 'telnet_server', 'action': 'login', 'status': 'failed', 'ip': self.transport.getPeer(
                        ).host, 'port': self.transport.getPeer().port, 'username': self._user.decode('utf-8'), 'password': self._pass.decode('utf-8')})
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'telnet_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'telnet_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'telnet_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
    ...
    def close_port(self):
        ret = close_port_wrapper(
            'telnet_server', self.ip, self.port, self.logs)
        return ret
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qtelnetserver = QTelnetServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
