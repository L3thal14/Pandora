...
class QSMBServer():
    ...
    def smb_server_main(self):
            ...
            def write(self, message):
                ...
                try:
                    if "Incoming connection" in message.strip() or "AUTHENTICATE_MESSAGE" in message.strip() or "authenticated successfully" in message.strip():
                        _q_s.logs.info(
                            {'server': 'smb_server', 'action': 'connection', 'msg': message.strip()})
                    elif ":4141414141414141:" in message.strip():
                        parsed = message.strip().split(":")
                        if len(parsed) > 2:
                            _q_s.logs.info(
                                {'server': 'smb_server', 'action': 'login', 'workstation': parsed[0], 'test': parsed[1]})
                except Exception as e:
                    _q_s.logs.error(
                        ["errors", {'server': 'smb_server', 'error': 'write', "type": "error -> " + repr(e)}])
        ...
        server = smbserver.SimpleSMBServer(
            listenAddress=self.ip, listenPort=self.port)
        # server.removeShare("IPC$")
        server.addShare('C$', dirpath, '', readOnly='yes')
        server.setSMB2Support(True)
        server.addCredential(self.username, 0, compute_lmhash(
            self.password), compute_nthash(self.password))
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'smb_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'smb_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'smb_server', 'action': 'setup', 'status': 'error', 'ip': self.ip,
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qsmbserver = QSMBServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
