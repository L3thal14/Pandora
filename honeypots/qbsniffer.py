...
class QBSniffer():
    def __init__(self, filter=None, interface=None, config=''):
        self.current_ip = ifaddresses(
            interface)[AF_INET][0]['addr'].encode('utf-8')
        self.current_mac = ifaddresses(
            interface)[AF_LINK][0]['addr'].encode('utf-8')
        self.filter = filter
                ...
                try:
                    ...
                    if "load" in _fields[layer]:
                        ...
                        if rsearch(self.common, raw_payloads[layer]):
                            _q_s.logs.info(
                                {'action': 'creds_check', "payload": raw_payloads[layer]})
                except Exception as e:
                    print(e)
                    _q_s.logs.error(
                        ["errors", {'error': 'capture_logic_1', "type": "error -> " + repr(e)}])

            try:
                if _q_s.method == "ALL":
                    try:
                        _q_s.logs.info({'action': 'all', 'ip': _q_s.current_ip, 'mac': _q_s.current_mac,
                                        'layers': _layers, 'fields': _fields, "payload": hex_payloads})
                    except Exception as e:
                        _q_s.logs.error(
                            ["errors", {'error': 'capture_logic_2', "type": "error -> " + repr(e)}])
                elif _q_s.method == "TCPUDP":
                    if packet.haslayer('IP') and len(hex_payloads) > 0 and packet['IP'].src != _q_s.current_ip:
                        if packet.haslayer('TCP'):
                            try:
                                _q_s.logs.info({'action': 'tcppayload', 'ip': _q_s.current_ip, 'mac': _q_s.current_mac, 'src_ip': packet['IP'].src,
                                                'src_port': packet['TCP'].sport, 'dst_ip': packet['IP'].dst, 'dst_port': packet['TCP'].dport, "raw_payload": raw_payloads, "payload": hex_payloads})
                            except Exception as e:
                                _q_s.logs.error(
                                    ["errors", {'error': 'capture_logic_3', "type": "error -> " + repr(e)}])
                        elif packet.haslayer('UDP'):
                            try:
                                _q_s.logs.info({'action': 'udppayload', 'ip': _q_s.current_ip, 'mac': _q_s.current_mac, 'src_ip': packet['IP'].src,
                                                'src_port': packet['UDP'].sport, 'dst_ip': packet['IP'].dst, 'dst_port': packet['UDP'].dport, "raw_payload": raw_payloads, "payload": hex_payloads})
                            except Exception as e:
                                _q_s.logs.error(
                                    ["errors", {'error': 'capture_logic_4', "type": "error -> " + repr(e)}])

                if packet.haslayer('IP') and packet.haslayer('ICMP') and packet['IP'].src != _q_s.current_ip:
                    _q_s.logs.info({'action': 'icmp', 'ip': _q_s.current_ip, 'mac': _q_s.current_mac, 'src_ip': packet['IP'].src, 'dst_ip': packet['IP'].dst,
                                    'ICMP_Code': packet['ICMP'].code, 'ICMP_Type': packet['ICMP'].type, 'ICMP_MSG': self.find_ICMP(packet['ICMP'].type, packet['ICMP'].code)})

                if packet.haslayer('IP') and packet.haslayer('TCP') and packet['IP'].src != _q_s.current_ip:
                    if packet['TCP'].flags == 2:
                        _q_s.logs.info({'action': 'tcpscan', 'ip': _q_s.current_ip, 'mac': _q_s.current_mac, 'src_ip': packet['IP'].src,
                                        'src_port': packet['TCP'].sport, 'dst_ip': packet['IP'].dst, 'dst_port': packet['TCP'].dport, "raw_payload": raw_payloads, "payload": hex_payloads})
                        send(IP(dst=packet['IP'].src, src=packet['IP'].dst) / TCP(dport=packet['TCP'].sport,
                                                                                  sport=packet['TCP'].dport, ack=(packet['TCP'].seq + 1), flags='SA'), verbose=False)

            except Exception as e:
                _q_s.logs.error(
                    ["errors", {'error': 'capture_logic_5', "type": "error -> " + repr(e)}])
    ...
    def run_sniffer(self, process=None):
        if process:
            self.process = Process(
                name='QSniffer_', target=self.scapy_sniffer_main)
            self.process.start()
        else:
...
if __name__ == "__main__":
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qsniffer = QSniffer(filter=parsed.filter,
                            interface=parsed.interface, config=parsed.config)
