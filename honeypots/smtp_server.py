    ...
    def smtp_server_main(self):
        ...
        class CustomSMTPChannel(SMTPChannel):
            def smtp_EHLO(self, arg):
                _q_s.logs.info({'server': 'smtp_server', 'action': 'connection',
                ...
                if self._SMTPChannel__greeting:
                    ...
                    self.push(
                        '250-{0} Hello {1}'.format(self._SMTPChannel__fqdn, arg))
            ...
            def smtp_AUTH(self, arg):
                try:
                    if arg.startswith('PLAIN '):
                        _, username, password = b64decode(
                            arg.split(' ')[1].strip()).decode("utf-8").split('\0')
                        if username == _q_s.username and password == _q_s.password:
                            _q_s.logs.info({'server': 'smtp_server', 'action': 'login', 'status': 'success',
                                            'ip': self.addr[0], 'port': self.addr[1], 'username': _q_s.username, 'password': _q_s.password})
                        else:
                            _q_s.logs.info({'server': 'smtp_server', 'action': 'login', 'status': 'faild',
                                            'ip': self.addr[0], 'port': self.addr[1], 'username': username, 'password': password})
                except Exception as e:
                    print(e)
                    _q_s.logs.error(["errors", {
                                    'server': 'smtp_server', 'error': 'smtp_AUTH', "type": "error -> " + repr(e)}])
            ...
            if auto:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({'server': 'smtp_server', 'action': 'process', 'status': 'success',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                    else:
                        self.logs.info({'server': 'smtp_server', 'action': 'process', 'status': 'error',
                                        'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
                else:
                    self.logs.info({'server': 'smtp_server', 'action': 'setup', 'status': 'error',
                                    'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password})
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qsmtpserver = QSMTPServer(ip=parsed.ip, port=parsed.port, username=parsed.username,
